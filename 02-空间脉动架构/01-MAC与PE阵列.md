...
- **你能帮上的最大的忙是"喂对形状"**：阵列物理尺寸固定（TPU 是 128×128），矩阵维度是 128 的整数倍时阵列坐满；不是整数倍时，缺的部分填零凑整——**填零的 PE 照常搏动、消耗时钟和功耗，产出为零**（这笔账在本模块第 5 节细算）。所以在 TPU 生态里"维度选 128 的倍数"是和 GPU 世界"访存要合并"同级别的铁律。
- **观测手段**：编译器的报告和性能工具给出"阵列利用率"（多大比例的 PE 拍数在做有效计算）——这是脉动阵列世界里第一个要看的健康指标，角色相当于 GPU 世界的占用率加带宽利用率。

## 6. 最新架构落点（知识截至 2026-01）

- **谷歌 TPU**：教科书实现。核心是 128×128 的脉动阵列（官方名 MXU，Matrix Multiply Unit），一颗芯片有多个。从 2016 年第一代到 v7（Ironwood，2025 公布），阵列这个核心结构基本未变——说明该范式在稠密矩阵乘上已接近最优，代际进步主要在存储、互连和数量。
...
**语境例句**："TPU 的 MXU 是 128×128 的 systolic array。" —— 意思是：它一拍能做一万六千多次乘加，但也意味着 128 对齐的形状税和"只吃规则计算"的脾气。
...
- 阵列做多大最优？128×128 与 16×16×16 两种选择背后的定量权衡？
- 部分和的累加精度（低精度输入、高精度累加）在 PE 电路里怎么实现？

---

*最后更新：2026-07-06（第二版：按写作规范重写，新增 2×2 逐拍跟踪例）*
