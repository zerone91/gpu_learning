...
差别只在"每秒传输次数"从哪来：**计算 lane 和字节 lane 用的是时钟频率**（并行接口还要看是不是双沿传输），**链路 lane 用的是符号率（GT/s）**。下面四个算例，四种 lane 各一个。
...
**这就是模块 08 第 03 节说的"寄存器堆是全芯片带宽最高的存储"的算法。** 它之所以能达到这个量级，正是因为它是 32 × 132 = 4224 条**物理上互相独立的窄通道**在并行工作（模块 08 第 02 节 §4 的 lane 切片），而不是某一块 SRAM 跑得特别快。
...
**两个要点。** 第一，**HBM3 规范支持到 6.4 Gb/s，而 H100 并没有跑满**——规范速率和产品速率是两回事，看规格表时要认产品实测值。第二，这里的"位宽"是**物理引脚数**，所以它直接受模块 07 讲的岸线密度和中介层布线能力约束——**想加带宽，要么提引脚速率，要么加引脚，而后者受封装物理限制**，这正是模块 07 那条主线。
...
1. **GT/s 不等于 Gb/s。** GT/s 是每秒的**符号**数。NRZ 编码一个符号携带 1 位，两者数值相同；但 **PAM4 一个符号携带 2 位**，这时 Gb/s 是 GT/s 的两倍。PCIe 6.0、以及现代高速 SerDes 大多用 PAM4。
...
- **模块 08 第 02 节 §4（lane 与寄存器堆接线）与第 03 节 §6（数据通路拓扑）**：那里讲计算 lane 这一端，本节 §7 把它和存储接口、链路上的 lane 放在同一套折算公式下对照。
...
### GT/s、Gb/s 与单向双向口径
**定义**：GT/s 是每秒传输的**符号**数，Gb/s 是每秒传输的**比特**数。NRZ 编码一个符号载 1 位，两者数值相同；PAM4 一个符号载 2 位，Gb/s 是 GT/s 的两倍。此外还要扣编码开销（128b/130b 约 1.5%，8b/10b 达 20%）与前向纠错开销，并确认厂商报的是单向还是双向合计。
...
*最后更新：2026-08-31（第一版：模块 08 第 5 节；含 32×32 crossbar 门数算例与平方律表、候选匹配合并电路、L2 访问拍数构成表、每比特搬运能量表、100 GB/s 的并行 PHY 与 SerDes 两笔账、格雷码异步 FIFO、lane 四种含义的对照与四个带宽算例）*
