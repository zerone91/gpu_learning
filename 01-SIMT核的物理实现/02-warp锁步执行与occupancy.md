...
举个例子。同一个线程里如果连续写四条语句：`a1 += x1*y1; a2 += x2*y2; a3 += x3*y3; a4 += x4*y4`，这四条乘加互相不依赖，可以流水式地一条接一条发射，不需要等前一条出结果。这样**一个 warp 自己就同时挂着四条"在飞"的指令**，相当于顶了四个只有单条依赖链的 warp。
...
*最后更新：2026-07-06（第三版：按写作规范全文重写，作为全库重写的样板）*
