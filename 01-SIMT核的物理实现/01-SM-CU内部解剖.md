...
│  │ 访存单元 (LSU)    │  │          │ │          │ │          │ │
│  └───────────────────┘  └──────────┘ └──────────┘ └──────────┘ │
│  ┌──────── shared memory / L1 缓存（全 SM 共享，~228 KB）─────┐ │
│  └─────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬──────────────────────────────────┘
                                ▼
                 L2 缓存（全芯片共享，~50 MB）
...
这是 SM 里最反直觉的部件。每个 SM 的寄存器堆有 **65536 个 32 位寄存器，合计 256 KB**——比旁边那块 228 KB 的 shared memory/L1 还大。在 CPU 的世界里这不可思议：CPU 每个核心的寄存器只有几十个。为什么差了三个数量级？
...
每个 SM 有约 228 KB 的片上快速存储，物理上是同一块 SRAM，可以配置成两种用法：一部分当 **L1 缓存**（硬件自动管理，和 CPU 的缓存一样）；一部分当 **shared memory**（**程序员显式管理**：你自己写代码决定把什么数据搬进来、什么时候丢掉）。
...
以上结构是各代 NVIDIA GPU 共同的骨架。近年的演化方向值得记三条：
...
跨厂商对照：AMD 的对应部件名是 CU（对应 SM）、wavefront（对应 warp，64 线程）、LDS（对应 shared memory）、Matrix Core（对应 Tensor Core）。名字全换、结构几乎一一对应。
...
### shared memory
**定义**：每个 SM 上约 228 KB 的片上快速存储中，交给程序员显式管理的那部分。同一线程块内的线程共享它，读写延迟约 20~30 个周期（对比显存的几百周期）。
...
*最后更新：2026-07-06（第四版：按写作规范重写；旧版的费曼推导思路保留，表达全部重写为完整叙述+例子）*
