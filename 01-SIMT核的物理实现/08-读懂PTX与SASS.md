...
- **Blackwell** 的 `tcgen05` 一族接棒 wgmma（形状更大、累加器进 TMEM），PTX 文档已公开，SASS 层命名以 nvdisasm 实际输出为准。这一族里有两个新的"点号段"值得按 §2 的槽位法拆一遍：
  - **`.cta_group::1` / `.cta_group::2`**——决定这条矩阵指令是一个线程块自己做，还是**和簇内配对的那个块一起做**（模块 03 第 1 节 §4.1）。写成 `::2` 时操作数在两个 SM 之间分片，搬运量减半。**这是第一个把"跨 SM 协作"编进指令名的修饰符。**
  - **块缩放相关的修饰符**——低精度格式的缩放因子从哪取、按什么块大小配对（模块 06 第 2 节 §4.6）。**它的出现意味着"元数据通路"第一次成了指令名的一部分。**
...
*最后更新：2026-09-01（第二版：§8 更新到 2026-08，补入 tcgen05 一族里两个新槽位（cta_group 与块缩放修饰符）的拆解，以及读 Blackwell SASS 时"累加器换地方了"这个必查项。第一版：2026-07-08）*
